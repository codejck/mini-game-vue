export default class slidingPuzzle {
    ...
    countInv(arr) {
        // 蛇形扁平二维数组, 然后计算逆序数
        let flatArr = arr.map((k, i) => i & 1 ? k.toReversed() : k).flat()
        let count = 0
    ...
    getPath () {
        ...
        for (let i of [...Array(this.n - 2)].map((_, i) => i < this.$x - 1 ? i : this.n + this.$x - 2 - i)) {
        ...
        for (let j of [...Array(this.m - 2)].map((k, j) => j < this.$y - 1 ? j : this.m + this.$y - 2 - j).concat(this.$y - 1)) {
